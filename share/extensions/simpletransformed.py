...
def computeBBox(aList,mat=[[1,0,0],[0,1,0]]):
    ...
    for node in aList:
        ...
        elif node.tag==inkex.addNS('rect','svg') or node.tag=='rect' or node.tag==inkex.addNS('image','svg') or node.tag=='image':
            w = float(node.get('width'))/2.
            h = float(node.get('height'))/2.
            x = float(node.get('x'))
            y = float(node.get('y'))
            C = [x + w , y + h ]
            applyTransformToPoint(m,C)
